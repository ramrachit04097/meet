...
const CtaFooter: React.FC<CtaFooterProps> = ({ onBookCall, onViewPricing, onOpenModal }) => {
  ...
  return (
    <section id="cta-footer-section" className="relative py-32 px-6 md:px-16 lg:px-24 text-center overflow-hidden">
      ...
      <div className="relative z-10">
        <h2 id="cta-heading" className="text-5xl md:text-6xl lg:text-7xl font-heading italic text-white tracking-tight leading-[0.85] max-w-3xl mx-auto mb-4">
          Your next website starts here.
        </h2>
        <p id="cta-subtext" className="text-white/60 font-body font-light text-sm md:text-base max-w-xl mx-auto mb-8">
        ...
        {/* Footer */}
        <div id="cta-footer-bar" className="mt-32 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/40 font-body font-light text-xs">
            &copy; 2026 Studio. All rights reserved.
          </p>
          <div className="flex items-center gap-6">
            {(["Privacy", "Terms", "Contact"] as const).map((link) => (
              <a
                key={link}
                href="#"
                onClick={(e) => {
                  e.preventDefault();
                  if (onOpenModal) {
                    onOpenModal(link.toLowerCase() as any);
                  }
                }}
